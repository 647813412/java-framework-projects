...
### Picture APIs (`/api/picture/*`)
| Function | Method | Path | Description |
|----------|--------|------|-------------|
| `listPictureVoByPageWithCacheUsingPost` | POST | `/api/picture/list/page/vo/cache` | 首页分页查询公开图片（带缓存） |
| `listPictureVoByPageUsingPost` | POST | `/api/picture/list/page/vo` | 分页查询公开图片 |
| `getPictureVoByIdUsingGet` | GET | `/api/picture/get/vo` | 根据ID获取图片详情VO |
| `listPictureTagCategoryUsingGet` | GET | `/api/picture/tag_category` | 获取标签和分类列表 |
| `deletePictureUsingPost` | POST | `/api/picture/delete` | 删除图片 |
| `editPictureUsingPost` | POST | `/api/picture/edit` | 编辑图片信息 |
| `uploadPictureUsingPost` | POST | `/api/picture/upload` | 上传本地图片文件（multipart/form-data） |
| `uploadUrlPictureUsingPost` | POST | `/api/picture/upload/url` | 通过URL上传图片（JSON） |

### User APIs (`/api/user/*`)
| Function | Method | Path | Description |
|----------|--------|------|-------------|
| `userLoginUsingPost` | POST | `/api/user/login` | 用户登录 |
| `userLogoutUsingGet` | GET | `/api/user/logout` | 用户登出 |
| `useRegisterUsingPost` | POST | `/api/user/register` | 用户注册 |
| `getUserLoginUsingGet` | GET | `/api/user/get/login` | 获取当前登录用户 |
| `getUserVoByIdUsingGet` | GET | `/api/user/get/vo` | 根据ID获取用户VO |
| `updateUserVoUsingPost` | POST | `/api/user/update/vo` | 用户更新自己的资料 |
| `listUserVoByPageUsingPost` | POST | `/api/user/list/page/vo` | 管理员分页查询用户 |

### File APIs (`/api/file/*`)
| Function | Method | Path | Description |
|----------|--------|------|-------------|
| `fileUploadUsingPost` | POST | `/api/file/upload` | 通用文件上传（头像等） |
| `uploadByUrlUsingPost` | POST | `/api/file/upload/url` | URL 上传文件 |

### Space APIs (`/api/space/*`)
| Function | Method | Path | Description |
|----------|--------|------|-------------|
| `addSpaceUsingPost` | POST | `/api/space/add` | 创建空间 |
| `listSpaceVoByPageUsingPost` | POST | `/api/space/list/page/vo` | 分页查询空间 |
| `listSpaceLevelUsingGet` | GET | `/api/space/list/level` | 获取空间级别列表 |
| `getSpaceVoByIdUsingGet` | GET | `/api/space/get/vo` | 获取空间详情 |
...
- 顶部横幅 + 搜索框（关键字搜索）
- 分类标签筛选栏
- 瀑布流图片展示（`listPictureVoByPageWithCacheUsingPost`）
- 点击图片跳转详情页（未登录则跳转登录页）
- 分页器

### PictureDetailView（图片详情）
- 左侧：图片预览（flex居中，`max-width: 100%`，圆角展示）
- 右侧：图片信息面板（标题、简介、分类标签、尺寸/比例/大小/格式/主色调）
- 上传者信息区，点击跳转用户主页（`/user/:id`）
- 点赞/分享/下载操作
- 数据来源：`getPictureVoByIdUsingGet`

### ProfileView（个人中心）
- 封面横幅 + 头像、昵称、账号、注册日期、简介
- 编辑资料弹窗（昵称/头像URL/上传/简介）
- 公共图库 Tab：我的公开图片 + 上传/编辑/删除
- 私有图库 Tab：私有空间管理 + 图片上传/编辑/删除
- 图片上传弹窗：支持本地文件上传（`uploadPictureUsingPost`）和URL上传（`uploadUrlPictureUsingPost`）

### UserProfileView（用户主页）
- 他人主页（`/user/:id`），展示用户公开信息和公开图片
- 数据来源：`getUserVoByIdUsingGet` + `listPictureVoByPageWithCacheUsingPost`

### 管理后台
- AdminUserManageView：用户管理（增删改查）
